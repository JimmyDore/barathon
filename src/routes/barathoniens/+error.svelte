...
<svelte:head>
	<title>{notFound ? 'Inconnu au bataillon' : 'Oups'} · Barathon</title>
</svelte:head>

<div class="container oops">
	<p class="code" aria-hidden="true">{page.status}</p>
	<h1>{notFound ? 'Inconnu au bataillon' : 'Ça a coincé'}</h1>
	<p class="muted">{page.error?.message ?? 'Réessaie dans un instant.'}</p>
	<div class="actions">
		<Button href="/barathoniens">Voir le classement</Button>
